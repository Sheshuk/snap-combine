...
#### Monitoring: [snap.util.monitor](snap/util/monitor.py)
...
#### Generating client data: [snap.client.fake](snap/client/fake.py)
...
#### Triggering: [snap.util.threshold](snap/util/threshold)

* `Threshold`: select and forward only portions of data with significance above given threshold

#### Misc: [snap.util](snap/util/)

* `dump_to_file`: Dump data to given file

## Example

Example configurations provided the [examples](examples/) dir use these steps to 
...
```
These commands will start producing the significance data from the two clients.

In another shell (combination side):
```
snap combine.yml
```

You should see the monitoring ticking for all nodes.
